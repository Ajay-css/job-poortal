...
const JobListeninig = () => {
  ...
  const [selectedLoactions, setSelectedLocations] = useState([])
  ...
  useEffect(() => {
    ...
    const matchesLocation = job => selectedLoactions.length === 0 || selectedLoactions.includes(job.location);
    ...
    setCurrentPage(1);
  }, [jobs, selectedCategories, selectedLoactions, searchFilter])
          ...
          isSearched && (searchFilter.title !== '' || searchFilter.location !== '') && (
            <>
              ...
              <div className='mb-4 text-gray-600'>
                {searchFilter.title && (
                  <span className='inline-flex items-center gap-2.5 bg-blue-50 border border-blue-200 rounded px-4 py-1.5'>
                    {searchFilter.title}
                    <img onClick={e => setSearchFilter(prev => ({ ...prev, title: "" }))} className='cursor-pointer' src={assets.cross_icon} alt="" />
                  </span>
                )}
                {searchFilter.location && (
                  <span className='ml-2 inline-flex items-center gap-2.5 bg-red-50 border border-red-200 rounded px-4 py-1.5'>
                    {searchFilter.location}
              ...
              JobLocations.map((location, index) => (
                <li className='flex gap-3 items-center' key={index}>
                  <input className='scale-125' type="checkbox"
                    ...
                    checked={selectedLoactions.includes(location)} />
                  {location}
                </li>
              ))
            }
